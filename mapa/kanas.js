export function Kanas(){
var hir =  {
    1:{"a":"&#12354;","i":"&#12356;","u":"&#12358;","e":"&#12360;","o":"&#12362;","n":"&#12435;"},
 2:{
 ...
 "sha":"&#12375;&#12419;","shu":"&#12375;&#12421;","sho":"&#12375;&#12423;","shi":"&#12375;","tsu":"&#12388;","dzu":"&#12389;",
...
var kat =  {
 ...
 "sha":"&#12471;&#12515;","shu":"&#12471;&#12517;","sho":"&#12471;&#12519;","shi":"&#12471;","tsu":"&#12484;","dzu":"&#12485;",
...
var rom = {
 ...
 12384:"da",12386:"ji",12389:"dzu",12391:"de",12393:"do",
 12480:"da",12482:"ji",12485:"dzu",12487:"de",12489:"do",
...
ddd.innerHTML=getHiragana("kana");
...
for ( key in hir){
console.log({key,hir:hir[key]});
let aprefix="";//key.charAt(0);
for ( akey in hir[key]){
let prefix=akey.charAt(0);
if(prefix!=aprefix && aprefix!='a' && prefix!='f'){str+="<br>";aprefix=prefix;}
console.log({akey,hir:hir[key][akey]});
kanas.hiragana.push({ro:akey,char:hir[key][akey]});
str+=`<div class='kanaChar'><b>${hir[key][akey]}</b><br><small>${akey}</small></div>`
}
str+="<br><br>";
}
str+="<hr>";
for ( key in kat){
console.log({key,kat:kat[key]});
aprefix="";
for ( akey in kat[key]){
let prefix=akey.charAt(0);
if(prefix!=aprefix && aprefix!='a' && prefix!='f'){str+="<br>";aprefix=prefix;}
...
console.log({akey,kat:kat[key][akey]});
kanas.katakana.push({ro:akey,char:hir[key][akey]});

str+=`<div class='kanaChar'><b>${kat[key][akey]}</b><br><small>${akey}</small></div>`
...
str+="<hr>";
ddd.innerHTML=str;
console.log(JSON.stringify(kanas,false,4));
});
...
for(var i=0;i<str.length;i++) {
 ...
 for(var j=0;j<tmpword.length;j++) {
     ...
     if (ch === 12387 || ch === 12483) { // っ или ッ
         var nextch = tmpword.charCodeAt(j + 1);
         var romaji = getr(nextch, rom);
         if (romaji) {
             res += romaji[0];
             continue;
         }
     ...
     if(tmpch) {
         ...
         res+='<span style="color:#cccccc">&#'+ch+';</span>';
     }
 }
 res+=' ';
}
...
var res = "";
for (var k = 0; k < str1.length; k++) {
var str = str1[k].split(" ");
for (var i = 0; i < str.length; i++) {
 var tmpstr = str[i];
 for (var j = 0; j < tmpstr.length; j++) {
     var tmpch = "";

     // Проверка на маленькую "цу" (っ или ッ)
     if (tmpstr[j] === tmpstr[j + 1] && !/[aeiou]/.test(tmpstr[j])) {
         tmpch = getc(4, "tsu", arr); // Добавляем маленькую "цу"
         res += tmpch;
     ...
     if (tmpch) {
         ...
         tmpch = getc(2, tmpstr.substring(j, j + 2).toLowerCase(), arr);
         if (tmpch) {
             j++;
         } else {
     ...
     if (tmpch) {
         ...
         res += `<span style="color:#aa6666">${tmpstr.substring(j, j + 1)}</span>`;
     }
 }
 res += " ";
}
